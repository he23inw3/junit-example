...
class UserRepositoryTest {
	...
	inner class Create {
		...
		fun create_case1() {
			...
			every { userMapper.bulkInsert(listOf(u001Record)) } returns Unit

			// execute
			testSuite.create(listOf(u001Entity), "junit")

			// verify
			verify { userMapper.bulkInsert(listOf(u001Record)) }
		}
	}
	...
	inner class Update {
		...
		fun update_case1() {
			...
			every { LocalDateTime.now() } returns now
			every { userMapper.update(u003Record) } returns Unit

			// execute
			testSuite.update(u003Entity, "junit")

			// verify
			verify { userMapper.update(u003Record) }
		}
	}
